...
While running = 1
	...
	Select Dead
		Case 0 
			Gosub updatemario
			Gosub testmariocollision;if we test collision first than spamming tap jump under that multicoin won't get game glitched?
			Gosub updateblocks
...
.updatepowerups
	For apowerup.powerup = Each powerup 
		...
		Else
			...
			If (apowerup\what = 3 Or apowerup\what = 5) And apowerup\stillappearing = 0 
			...
			EndIf
		EndIf
		If apowerup\what = 3 And apowerup\stillappearing = 0 ;vertical collision code for the Big mario mushroom.......hnnnnnnnng there's a more efficent way to do this ,adn to combine code for this and the stars, but it's too hard to do it in BB (for minimal gain)...if C++ had worked....
